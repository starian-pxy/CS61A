...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def func(wenduan):
        disposed=split(lower(remove_punctuation(wenduan)))
        result=[i for i in disposed if i in subject]
        if result==[]:
            return False
        else:
            return True
...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words),len(source_words))):
        if typed_words[i] == source_words[i]:
                k+=1
    return 100.0 * k / len(typed_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    diction={diff_function(typed_word,word_list[x],limit):word_list[x] for x in range(len(word_list)-1,-1,-1)}
    if typed_word in word_list:
        return typed_word
    else:
        return diction[min(diction.keys())] if min(diction.keys()) <= limit else typed_word
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    return dp[a][b] if dp[a][b] <= limit else limit + 1
...
def report_progress(typed, source, user_id, upload):
    ...
    while (i<len(typed) and typed[i] == source[i]):
        i += 1
    diction={'id': user_id, 'progress': i/len(source)}
    upload(diction)
    return i/len(source)
    # END PROBLEM 8
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match,min,i)>time(match,j,i):
                min=j
        dicton[i]=min
    list=[]
    for i in player_indices:
        list.append([get_word(match,key) for key,value in dicton.items() if value == i])
    return list
    # END PROBLEM 10
